...
def generate_fid_stat(absolute_file_directory, IMAGE_SIZE = 256):
    ...
    for images in os.listdir(absolute_file_directory):
        img_tensor.append(self.transforms(PIL.Image.open(os.path.join(YOLO_DIR, 'raw_dataset', row["dataset-path"])).convert("RGB")))
    
    return img_tensor


fid_stat = generate_fid_stat(os.path.join(YOLO_DIR, 'raw_dataset', 'train', 'images'))
